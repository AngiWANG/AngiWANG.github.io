...
| executor                           | core-threads | queue | max-threads | keepalive-time | blocking |
| ---------------------------------- | ------------ | ----- | ----------- | -------------- | -------- |
| thread-factory                     |              |       |             |                |          |
...
A thread pool executor with a bounded queue, where threads attempting to submit tasks may block. Such a thread pool has a core and maximum size and a specified queue length.  When a task is submitted,
​                 if the number of running threads is less than the core size, a new thread is created.  Otherwise, if
​                 there is room in the queue, the task is enqueued. Otherwise, if the number of running threads is less
