...
```shell
$ mvn dependency:copy-dependencies -DoutputDirectory=target -DincludeScope=compile
$ mvn dependency:copy-dependencies -DoutputDirectory=target -DincludeScope=compile -Dmdep.useRepositoryLayout=true
```

| 参数                               | 描述                 |
| -------------------------------- | ------------------ |
| -Dmdep.useRepositoryLayout =true | 按坐标目录结构            |
| -DoutputDirectory=target         | 导出到工程目录下的target目录下 |
| -DincludeScope=compile           | 导出指定级别的依赖          |
