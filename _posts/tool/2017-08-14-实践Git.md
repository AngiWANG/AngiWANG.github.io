...
也可以后面通过TortioseGit的settings中设置，参见下图：
![Tortiose-AutoCrlf](/images/Tortiose-AutoCrlf.png)

且统一换行符为Unix换行符，参见[Eclipse安装和配置](Eclipse安装和配置)

安装完成后把Git for Windows的git加入到环境变量path中，命令行验证git安装是否成功。
```
C:\Users\Angi>git --version
...
Linux UTF-8环境下，git的中文支持已经非常好了，但是**默认设置下**，中文文件名在工作区状态输出，查看历史更改概要，以及在补丁文件中，文件名的中文不能正确地显示，而是显示为八进制的字符编码，示例如下：
